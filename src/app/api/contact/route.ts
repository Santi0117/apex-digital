  ...
  try {
    if (!isDatabaseConfigured()) {
      return NextResponse.json(
        {
          error:
            "El formulario aún no está configurado. Escribinos por WhatsApp.",
        },
        { status: 503 },
      );
    }
    ...
    if (typeof body.website === "string" && body.website.trim()) {
      return NextResponse.json({
        ok: true,
        message:
          "Recibimos tu mensaje. Te contactamos en menos de 24 horas hábiles.",
      });
    }
    ...
    if (!interest || interest.length < 10) {
      return NextResponse.json(
        {
          error:
            "Contanos brevemente tu motivo o interés (mín. 10 caracteres).",
        },
        { status: 400 },
      );
    }
    ...
    if (
      ...
      !dbError.message.includes("service")
    ) {
    ...
    if (dbError) {
      ...
      return NextResponse.json(
        {
          error:
            "No se pudo guardar tu consulta. Intentá de nuevo o escribinos por WhatsApp.",
        },
        { status: 500 },
      );
    }
