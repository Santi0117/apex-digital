...
export default function PillNav({
  ...
  const socialTweenRef = useRef<gsap.core.Tween | null>(null);
  ...
  }, [items, ease, initialLoadAnimation, socialHref]);
  ...
  const handleSocialEnter = () => {
    ...
    if (!el) return;
    socialTweenRef.current?.kill();
    gsap.set(el, { rotate: 0 });
    socialTweenRef.current = gsap.fromTo(
      el,
      { rotate: 0 },
      { rotate: 360, duration: 0.55, ease, overwrite: "auto" },
    );
  };
  ...
  return (
    <div className="pill-nav-container">
      ...
      >
        ...
        {socialHref ? (
          <a
            className="pill-logo pill-social"
            href={socialHref}
            target="_blank"
            ...
            ref={socialRef}
          >
            <InstagramGlyph className="pill-social-icon" />
          </a>
        ) : null}
      </nav>
      ...
      >
        <ul className="mobile-menu-list">
          {items.map((item, i) => {
            const active = activeHref === item.href;
            const classNameLink = `mobile-menu-link${active ? " is-active" : ""}`;
            if (!isExternalLink(item.href)) {
              return (
